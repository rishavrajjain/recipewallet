# main.py – FastAPI backend (June 2025) - Modified for Render Deployment
...
# NOTE: Ensure these model names are correct for your OpenAI account.
# As of mid-2025, standard models might be "gpt-4o" and "dall-e-3".
CHAT_MODEL = "gpt-4.1"
...
# Serve generated images from the temporary directory.
# NOTE: Render's free tier has an ephemeral filesystem. Files in /tmp
# will be deleted when the service spins down or restarts.
...
def transcribe(audio_path: Path) -> str:
    # NOTE: 'gpt-4o-transcribe' appears to be a custom model name.
    # The standard OpenAI model for this is 'whisper-1'.
...
async def generate_step_image(idx: int, comp: Dict[str, str]) -> Dict[str, str]:
    ...
    if url is None and hasattr(rsp.data[0], "b64_json"):
        ...
        # <--- MODIFIED FOR RENDER --- >
        # Use Render's external URL env var if available, otherwise fallback to localhost.
...
# The __main__ block is good for local testing but is not used by Render's server.
