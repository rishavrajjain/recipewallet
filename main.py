...
def run_yt_dlp(url: str, dst: Path) -> dict:
    ...
    # Optional: Session cookies via env vars (improves success rate if login is required)
    cookies_path = None
    if "instagram.com" in url.lower():
        ...
        tiktok_webid = os.getenv("TIKTOK_WEBID", "").strip()
        if tiktok_sessionid or tiktok_webid:
            try:
                cookies_path = str(dst / "tiktok_cookies.txt")
                expiry = int(time.time()) + 3600 * 24 * 365  # 1 year
                with open(cookies_path, "w") as f:
                    f.write("# Netscape HTTP Cookie File\n")
                    # domain, include_subdomains, path, secure, expiry, name, value
                    if tiktok_sessionid:
                        f.write(f".tiktok.com\tTRUE\t/\tTRUE\t{expiry}\tsessionid\t{tiktok_sessionid}\n")
                    if tiktok_webid:
                        f.write(f".tiktok.com\tTRUE\t/\tTRUE\t{expiry}\twebid\t{tiktok_webid}\n")
                print(f"TikTok session cookies configured (sessionid: {bool(tiktok_sessionid)}, webid: {bool(tiktok_webid)})")
            except Exception as e:
                print(f"Failed to write TikTok cookies file: {e}")
